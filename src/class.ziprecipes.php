...
class ZipRecipes {
    ...
    public static function zrdn_format_recipe($recipe) {
        ...
        if ($recipe->serving_size != null || $recipe->calories != null || $recipe->fat != null || $recipe->carbs != null || $recipe->protein != null || $recipe->fiber != null || $recipe->sugar != null || $recipe->saturated_fat != null || $recipe->sodium != null) {
    ...
    public static function shortcode_recipe($data, $atts) {
        ...
        $callback_name = 'recipe';

        $shortcode_type = array_shift($atts);

        if ($shortcode_type != $callback_name) {
            return $data;
        }
        if (isset($atts['id']) && !empty($atts['id'])) {
            $recipe = self::zrdn_select_recipe_db($atts['id']);
            return self::zrdn_format_recipe($recipe);
        }
    }
